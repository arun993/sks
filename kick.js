        ...
        return new Promise((resolve, reject) => {
            ...
            const getTaskList = () => {
                if (!hasCalledGetTaskList) {
                    this.log('Sending request getTaskList', 'info');
                            ...
                            if (parsed[0].list) {
                                ...
                                if (todaySignin) {
                                    if (todaySignin.isSignin === false) {
                                        this.log('Sending signin request', 'info');
                                        const signinMessage = '426["signin"]';
                                        ws.send(signinMessage);
                                    } else {
                                        this.log('You have already signed in today', 'info');
                                        getTaskList();
                                    }
                                } else {
                                    this.log('Could not find today in the list', 'warning');
                                    getTaskList();
                                }
                                ...
                                this.log(`Action failed: ${parsed[1].message}`, 'error');
                                getTaskList();
                                ...
                                if (tasks.some(task => task.hasOwnProperty('isFinish'))) {
                                    ...
                                    this.log(`Found ${unfinishedTasks.length} unfinished tasks`, 'info');
                                
                                    for (const task of unfinishedTasks) {
                                        await new Promise(resolve => setTimeout(resolve, 1000));
                                        const changeTaskMessage = `42${Math.floor(Math.random() * 1000)}["changeTask",{"taskId":"${task._id}"}]`;
                                        ws.send(changeTaskMessage);
                                        this.log(`Doing task: ${task.title}`, 'info');
                                    }
    
                                    setTimeout(closeWebSocket, 3000);
                                } else {
                                    this.log('Received list, but not a task list', 'info');
                                    closeWebSocket();
                                }
                            }
                        } else {
                            this.log(`Invalid message format: ${JSON.stringify(parsed)}`, 'warning');
                        }
                    } catch (error) {
                        this.log(`Error processing message: ${error.message}`, 'error');
                        this.log(`Original message content: ${message}`, 'error');
                    }
                }
            });
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (loginResult.success) {
                    ...
                    if (userDateResult.success) {
                        this.log(`Account age: ${userDateResult.displayYears} years`, 'info');
                        this.log(`Reward: ${userDateResult.reward}`, 'info');
                    } else {
                        this.log(`Failed to get account age: ${userDateResult.error}`, 'error');
                    }
                    this.log('Reading quest data...', 'info');
                    try {
                    ...
                    this.log(`Failed to login! ${loginResult.error}`, 'error');
                }

                await new Promise(resolve => setTimeout(resolve, 1000));
            }

            await this.countdown(1380 * 60);
        }
    }
}
