...
class SidekickAPIClient {
    ...
    async getUserDate(token, telegramId, proxy) {
    ...
    async connectWebSocket(token, proxy) {
    ...
    async main() {
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                try {
                    const loginResult = await this.login(init, proxy);
                    if (loginResult.success) {
                        this.log('Đăng nhập thành công!', 'success');
                        const token = loginResult.token;
                        
                        const userDateResult = await this.getUserDate(token, telegramId, proxy);
                        ...
                        try {
                            await this.connectWebSocket(token, proxy);
                        } catch (error) {
                            this.log(`Lỗi rồi: ${error.message}`, 'error');
                        }
                    } else {
